...
docker cp "$CSV" project-three-postgres:/tmp/AirQualityUCI.csv
bash run_migrations.sh

docker exec -i project-three-postgres psql -U "${POSTGRES_USER:-iot}" -d "${POSTGRES_DB:-iot_events}" -c "TRUNCATE staging_airquality;"
docker exec -i project-three-postgres psql -U "${POSTGRES_USER:-iot}" -d "${POSTGRES_DB:-iot_events}" -c "COPY staging_airquality (date_col,time_col,co_gt,pt08_s1_co,nmhc_gt,c6h6_gt,pt08_s2_nmhc,nox_gt,pt08_s3_nox,no2_gt,pt08_s4_no2,pt08_s5_o3,temperature,rh,ah,extra1,extra2) FROM '/tmp/AirQualityUCI.csv' WITH (FORMAT csv, HEADER true, DELIMITER ';', NULL '-200');"
docker exec -i project-three-postgres psql -U "${POSTGRES_USER:-iot}" -d "${POSTGRES_DB:-iot_events}" -q < 0002_transform.up.sql
